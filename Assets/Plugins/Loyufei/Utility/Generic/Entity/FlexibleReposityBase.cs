...
namespace Loyufei
{
        ...
        where TReposit : IReposit<TData>
    {
        ...
        public IEnumerable<IReposit<TData>> Create(int amount)
        {
            ...
            var count  = Limited ? amount.Clamp(0, limit) : amount;
        ...
        public bool Insert(int index, IReposit<TData> reposit)
        {
            ...
            var insert = Activator.CreateInstance<TReposit>();
            
            insert.Set(reposit.Identity);
            insert.Preserve(reposit.Data);

            _Reposits.Insert(index, insert);
